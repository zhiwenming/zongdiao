...
import model.DangRiZhuangLvYueLv;
public class DangRiZhuangLvYueLvReader {
...
//	       System.out.println(reader.);
	       ...
	       reader.close();
	       List<DangRiZhuangLvYueLv> dList = new ArrayList<>();
	       for (int row = 0;row < List.size(); row++) {
	          int Length=List.get(row).length;
	          if(Length > 0){
	        	  DangRiZhuangLvYueLv dr = new DangRiZhuangLvYueLv();
