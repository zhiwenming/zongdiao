...
import model.ZhangAiXiuFuShiChang;
public class ZhangAiXiuFuShiChangReader {
...
//	       System.out.println(reader.);
	       ...
	       reader.close();
	       List<ZhangAiXiuFuShiChang> dList = new ArrayList<>();
	       for (int row = 0;row < List.size(); row++) {
	          int Length=List.get(row).length;
	          if(Length > 0){
	        	  ZhangAiXiuFuShiChang dr = new ZhangAiXiuFuShiChang();
	        	  ...
	        	  dr.k_l=List.get(row)[11];
	        	  dr.k_m=List.get(row)[12];
	        	  dList.add(dr);
