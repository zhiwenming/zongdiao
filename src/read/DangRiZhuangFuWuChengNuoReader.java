...
import model.DangRiZhuangFuWuChengNuo;
public class DangRiZhuangFuWuChengNuoReader {
...
//	       System.out.println(reader.);
	       ...
	       reader.close();
	       List<DangRiZhuangFuWuChengNuo> dList = new ArrayList<>();
	       for (int row = 0;row < List.size(); row++) {
	          int Length=List.get(row).length;
	          if(Length > 0){
	        	  DangRiZhuangFuWuChengNuo dr = new DangRiZhuangFuWuChengNuo();
