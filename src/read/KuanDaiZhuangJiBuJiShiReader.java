...
import model.KuanDaiZhuangJiBuJiShi;
public class KuanDaiZhuangJiBuJiShiReader {
...
//	       System.out.println(reader.);
	       ...
	       reader.close();
	       List<KuanDaiZhuangJiBuJiShi> dList = new ArrayList<>();
	       for (int row = 0;row < List.size(); row++) {
	          int Length=List.get(row).length;
	          if(Length > 0){
	        	  KuanDaiZhuangJiBuJiShi dr = new KuanDaiZhuangJiBuJiShi();
